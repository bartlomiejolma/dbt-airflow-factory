...
class DbtAirflowGraph:
    ...
    def add_exposure_tasks(self, manifest: dict) -> None:
        ...
        for exposure in manifest_exposures:
            logging.info(f"Creating exposure for {exposure['name']}")
            self._add_exposure_node(exposure, manifest=manifest)
    ...
    def create_edges_from_dependencies(
        ...
        for graph_node_name, graph_node in self.graph.nodes(data=True):
            if (
                graph_node["node_type"] == NodeType.MULTIPLE_DEPS_TEST
                and not include_contract_tests_dependencies
            ):
                continue
    ...
    def _add_exposure_node(self,         manifest_node: Dict[str, Any],
        manifest: dict,) -> None:
        dataset = manifest_node["meta"]["dataset"][Variable.get("env")]
        self.graph.add_node(
            manifest_node["name"],
            select=manifest_node["name"],
